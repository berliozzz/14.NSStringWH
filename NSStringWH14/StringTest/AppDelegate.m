...
//  StringTest
//
//  Created by Nikolay Berlioz on 23.09.15.
...
//    NSLog(@"----------------   First half   -------------------\n\n\n");
//    
//    NSLog(@"%@", [text substringToIndex:[text length] / 2]);
//
//    NSLog(@"----------------   Second half   -------------------\n\n\n");
//
//    NSLog(@"%@", [text substringFromIndex:[text length] / 2]);

    NSRange searchRange = NSMakeRange(0, [text length]);
    NSRange pieceRange = NSMakeRange( 0, 0);
    ...
    while (YES)
    {
        ...
        NSRange pieceRange = NSMakeRange(searchRange.location, (range.location + range.length) - searchRange.location );
        if (range.location != NSNotFound)
        {
            NSInteger index = range.location + range.length;
            searchRange.location = index;
            searchRange.length = (NSInteger)[text length] - index;
            [mArray addObject:[text substringWithRange:pieceRange]];
        }
        else
        {
            break;
...
//    NSLog(@"%@", mArray);
    ...
    NSMutableArray *arrayWithNSStringWord = [[NSMutableArray alloc] init];
    
    NSRange subRange;
    
    for (NSString* obj in mArray)
    {
        subRange = [obj rangeOfString:@"NSString"];
        if (subRange.location != NSNotFound)
        {
            [arrayWithNSStringWord addObject:obj];
        }
    }
    
    NSLog(@"%@", arrayWithNSStringWord);
